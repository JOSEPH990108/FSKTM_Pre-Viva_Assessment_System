                            ...
                            <td class="align-middle">
                                <div class="action-button">
                                    <?php
                                        ...
                                        if($application_status == 'APPROVED' && $row1 != NULL && $rowcount != $number_of_marker){
                                        ...
                                        if($application_status ==  'PRESENTATION'){
                                            $sql4  = "SELECT pc.*, pr.* FROM tbl_presentation_comment pc
                                            ...
                                            if(!mysqli_stmt_prepare($stmt4, $sql4)){
                                                ...
                                                }else if($row2 != NULL && $rowcount1 != $number_of_marker){
                                        ...
                                        if($application_status == "COMPLETED" && $rowcount == $number_of_marker){
            ...
            <table class="table table-striped table-bordered table-hover dataTables-examiner" style="width: 100%">
                    <thead>
                        <tr>
                            <th>Actions</th>
                            <th class="d-none">Application ID</th>
                            <th class="d-none">As Examiner</th>
                            <th>Matric No</th>
                            <th class="d-none">Student Name</th>
                            <th class="d-none">Level of Study</th>
                            <th class="d-none">Programme</th>
                ...
                <tbody>
                    <?php
                        $sql1 = "SELECT a.*, stu.level_of_study, stu.programme, stu.research_title, u.name, u.user_id, e.examiner1, e.examiner1_status, e.examiner2, e.examiner2_status 
                                FROM 
                                    (((tbl_application a
                                    INNER JOIN tbl_student stu ON a.student_id = stu.student_id)
                                    INNER JOIN tbl_user u ON stu.user_id = u.user_id)
                        ...
                        if(!mysqli_stmt_prepare($stmt, $sql1)){
                            ...
                            while($row1 = mysqli_fetch_assoc($result1)){
                                $application_id1 = $row1['application_id'];
                                $matric_no1 = $row1['user_id'];
                                $thesis1 = $row1['thesis_file'];
                                $plagiarism1 = $row1['plagiarism'];
                                $proofread1 = $row1['proofread'];
                                $pre_viva_date1 = $row1['pre_viva_date'];
                                $pre_viva_time1 = $row1['pre_viva_time'];
                                $pre_viva_venue_platform1 = $row1['pre_viva_venue_platform'];
                                $pre_viva_venue1 = $row1['pre_viva_venue'];
                                $application_status1 = $row1['application_status'];
                                $overall_mark1 = $row1['overall_mark'];
                                $application_result1 = $row1['application_result'];
                                $remark1 = $row1['remark'];
                                $level_of_study1 = $row1['level_of_study'];
                                $programme1 = $row1['programme'];
                                $research_title1 = $row1['research_title'];
                                $student_name1 = $row1['name'];
                                $examiner1 = $row1['examiner1'];
                                $examiner2 = $row1['examiner2'];
                                $examiner1_status = $row1['examiner1_status'];
                                $examiner2_status = $row1['examiner2_status'];
                                $pre_viva_date_time1 = $pre_viva_date1 . ' ' . $pre_viva_time1;
                                $pre_viva_date_time1 = strtotime($pre_viva_date_time1);
                                $current_date_time1 = date('Y-m-d h:i:sa', time());
                                $current_date_time1 = strtotime($current_date_time1);
                                if($examiner1 == $userid){
                                    $examiner = "Examiner 1";
                                ...
                                $sql2 = "SELECT tc.*, tr.* FROM ((tbl_application a
                                         INNER JOIN tbl_thesis_comment tc ON a.application_id = tc.application_id)
                                         INNER JOIN tbl_thesis_result tr ON a.application_id = tr.application_id)
                                         WHERE a.application_id = ? AND (tc.user_id = ? AND tr.user_id = ?);";
                                $stmt2 = mysqli_stmt_init($conn);
                                ...
                                if(!mysqli_stmt_prepare($stmt2, $sql2)){
                                    ...
                                    mysqli_stmt_bind_param($stmt2, "sss", $application_id1, $userid, $userid);
                                    mysqli_stmt_execute($stmt2);
                                    $result2 = mysqli_stmt_get_result($stmt2);
                                    $row2 = mysqli_fetch_assoc($result2);
                                    mysqli_stmt_close($stmt2);
                                ...
                                $sql3 ="SELECT sup.user_id AS supervisor,
                                            CASE 
                                                WHEN e1.examiner_id IS NOT NULL THEN e1.user_id ELSE NULL 
                                            END AS examiner1,
                                            CASE 
                                ...
                                if(!mysqli_stmt_prepare($stmt3, $sql3)){
                                    ...
                                    mysqli_stmt_bind_param($stmt3, "s", $application_id1);
                                    mysqli_stmt_execute($stmt3);
                                    $result3 = mysqli_stmt_get_result($stmt3);
                                    $row3 = mysqli_fetch_assoc($result3);
                                    $supervisor = $row3['supervisor'];
                                    $examiner1 = $row3['examiner1'];
                                    $examiner2 = $row3['examiner2'];
                                    mysqli_stmt_close($stmt3);
                                    ...
                                    $sql4 = "SELECT tc.*, tr.* FROM tbl_thesis_comment tc
                                             INNER JOIN tbl_thesis_result tr ON (tc.application_id = tr.application_id AND tc.user_id = tr.user_id)
                                             WHERE tc.application_id = ? AND tr.application_id = ?;";
                                    $stmt4 = mysqli_stmt_init($conn);
                                    ...
                                    if(!mysqli_stmt_prepare($stmt4, $sql4)){
                                        ...
                                        mysqli_stmt_bind_param($stmt4, "ss", $application_id1, $application_id1);
                                        mysqli_stmt_execute($stmt4);
                                        $result4 = mysqli_stmt_get_result($stmt4);
                                        $rowcount = mysqli_num_rows($result4);
                                        mysqli_stmt_close($stmt4);

                                        //Check presentation assessment
                                        $sql5 = "SELECT tpc.*, tpr.* FROM ((tbl_application a
                                        ...
                                        if(!mysqli_stmt_prepare($stmt5, $sql5)){
                                            ...
                                            mysqli_stmt_bind_param($stmt5, "sss", $application_id1, $userid, $userid);
                                            mysqli_stmt_execute($stmt5);
                                            $result5 = mysqli_stmt_get_result($stmt5);
                                            $row5 = mysqli_fetch_assoc($result5);
                                            mysqli_stmt_close($stmt5);

                                            $sql6 = "SELECT tpc.*, tpr.* FROM (tbl_presentation_comment tpc 
                                                     INNER JOIN tbl_presentation_result tpr ON tpc.application_id = tpr.application_id)
                                                     WHERE tpc.application_id = ? AND tpr.application_id = ?;";
                                            ...
                                            if(!mysqli_stmt_prepare($stmt6, $sql6)){
                                                ...
                                                mysqli_stmt_bind_param($stmt6, "ss", $application_id1, $application_id1);
                        ...
                        <tr>
                            <td class="align-middle">
                                <div class="action-button">
                                    <?php
                                    if($application_status1 == "PENDING" && $examiner_status == "PENDING"){
                                    ...
                                    if($application_status1 == "PENDING" && $examiner_status == "ACCEPTED"){
                                        echo '<button class="btn btn-success" disabled>ACCEPTED</button>';
                                    }
                                    if($application_status1 == "PENDING" && $examiner_status == "REJECTED"){
                                        echo '<button class="btn btn-danger" disabled>REJECTED</button>';
                                    }
                                    if($application_status1 == 'APPROVED' && $row2 == NULL){
                                    ...
                                    if($application_status1 == 'APPROVED' && $row2 != NULL && $rowcount != $number_of_marker){
                                    ...
                                    if($application_status1 == 'PRESENTATION' && $row5 == NULL && ($current_date_time1 > $pre_viva_date_time1)){
                                        echo '<button class="btn btn-primary presentationassessmentbtn" data-toggle="modal" data-target="#presentationassessmentModal" data-tooltip="tooltip" title="Presentation Assessment"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button>'; 
                                    }
                                    if($application_status1 == 'PRESENTATION' && $row5 != NULL && $rowcount2 != ($number_of_marker-1)){
                                        echo '<button class="btn btn-warning presentationassessmenteditbtn" data-toggle="modal" data-target="#presentationassessmentModal" data-tooltip="tooltip" title="Edit Presentation Assessment"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button>'; 
                                    }
                                    if($application_status1 == "COMPLETED" && $rowcount == $number_of_marker){
                                        echo '<button class="btn btn-success view_overall_markbtn" data-toggle="modal" data-target="#viewoverallthesismarkModal" data-tooltip="tooltip" title="View"><i class="fa fa-eye" aria-hidden="true"></i></button>';
                                    }
                                    ?>
                                </div>
                            </td>
                            <td class="d-none"><?=$application_id1?></td>
                            <td class="d-none"><?=$examiner?></td>
                            <td class="d-none"><?=$userid?></td>
                            <td><?=$matric_no1?></td>
                            <td class="d-none"><?=$student_name1?></td>
                            <td class="d-none"><?=$level_of_study1?></td>
                            <td class="d-none"><?=$programme1?></td>
                            <td><?=$research_title1?></td>
                            <td class="align-middle">
                                <div class="action-button">
                            ...
                            <td class="align-middle"><?=$pre_viva_date1?></td>
                            <td class="align-middle text-center"><?=$pre_viva_time1?></td>
                            <td class="align-middle text-center"><?=$pre_viva_venue_platform1 . '<br>' . $pre_viva_venue1?></td>
                            <td class="align-middle">
                                <div class="action-button">
                                    <?php
                                        if($application_status1 == "PENDING"){
                                            echo '<button class="btn btn-secondary" disabled>PENDING</button>';
                                        }
                                        else if($application_status1 == "APPROVED"){
                                            echo '<button class="btn btn-success" disabled>APPROVED</button>';
                                        }
                                        else if($application_status1 == "PRESENTATION"){
                                            echo '<button class="btn btn-warning" disabled>PRESENTATION</button>';
                                        }
                                        else if($application_status1 == "REJECTED"){
                                            echo '<button class="btn btn-danger" disabled>REJECTED</button>';
                                        }
                                        else if($application_status1 == "COMPLETED"){
                                            echo '<button class="btn btn-success" disabled>COMPLETED</button>';
                                        }
                                    ?>
                                </div>
                            ...
                            <td class="align-middle">
                                <div class="action-button">
                                   <?php
                                        if($application_status1 == "COMPLETED" && $overall_mark1 != NULL){
                                            echo $overall_mark1 . '%';
                                        }
                                        else if(($application_status1 == "APPROVED" || $application_status1 =="PRESENTATION") && $overall_mark1 == NULL){
                                            echo '<button class="btn btn-warning" disabled>ASSESSING</button>';
                                        }
                                   ?>
                                </div>
                            ...
                            <td class="align-middle">
                                <div class="action-button">
                                    ...
                                    <?php
                                        if($application_result1 == NULL){
                                            if(($application_status1 == "APPROVED" || $application_status1 == "PRESENTATION") && $application_result1 == NULL){
                                                echo '<button class="btn btn-warning" disabled>ASSESSING</button>';
                                            }
                                        }
                                        else{
                                            if($application_status1 == "COMPLETED" && $application_result1 == 1){
                                                echo '<button class="btn btn-success" disabled>PASSED</button>';
                                            }
                                            else if($application_status1 == "COMPLETED" && $application_result1 == 0){
                                                echo '<button class="btn btn-danger" disabled>FAILED</button>';
